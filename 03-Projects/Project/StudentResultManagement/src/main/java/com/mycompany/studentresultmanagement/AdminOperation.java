...
public class AdminOperation implements Serializable{
    ...
    public static void addStudentOperation(Data data) {
        ...
        while (true) {
            try {
                ...
                if (stdExists) {
                    throw new InputUserExists("- Student ID already exists!");
                }
    ...
    public static void addAdminOperation(Data data) {
        String name = getValidString("Enter name: ");
        String password = getValidString("Create password: ");

        try {

            boolean adminExists = data.getAdmins().stream()
                    .anyMatch(admin -> admin.getName().equals(name));

            if (adminExists) {
                throw new InputUserExists("- Admin already exists!");
            }

            Admin adminNew = new Admin(name, password);
            data.addAdmin(adminNew);
            System.out.println("- Sucessfully added admin");
    ...
    public static void removeAdminOperation(Data data) {
        String name = getValidString("Enter Admin Name: ");
        
        Optional<Admin> admin = data.getAdmins().stream()
                .filter(a -> a.getName().equals(name))
                .findFirst();
        
        
        if (admin.isPresent()) {
            Admin adminToRemove = admin.get();
            data.removeAdmin(adminToRemove);
            System.out.println("- Sucessfully removed admin");
        }
        else {
            System.out.printf("- Admin [%s] not found!\n", name);
        }
    }
    
    public static void viewAdminOperation(Data data) {
        
        if (data.getAdmins().size() == 0) {
            System.out.println("- Insufficient admins to display!");
            return;
        }
